...
async describeData(params) {
  try {
    ...
    if (response.status == 200) {
      ...
      const statisticsResponse = await axios.post(`http://127.0.0.1:5001/describe`, data);
...
async performAnova(params) {
  try {
    ...
    // Send the fetched data to the statistics API endpoint
    const statisticsResponse = await axios.post(`http://127.0.0.1:5001/anova`, data);
...
async performTukeyTest(params) {
  try {
    ...
    // Send the fetched data to the statistics API endpoint
    const statisticsResponse = await axios.post(`http://127.0.0.1:5001/tukey`, data);
...
async performTTest(params) {
  try {
    ...
    // Send the fetched data to the statistics API endpoint
    const statisticsResponse = await axios.post(`http://127.0.0.1:5001/t_test`, data);
